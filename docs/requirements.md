...
- `RF-06` O sistema deve permitir criar, listar, consultar e atualizar famílias.
- `RF-07` O sistema deve permitir arquivar famílias por exclusão lógica, registrando data de arquivamento.
- `RF-08` O sistema deve controlar transições de status de família entre `Draft`, `Reviewed`, `Exported` e `Archived`.
- `RF-09` O sistema deve limitar a visualização de famílias para usuários `staff` às famílias criadas por eles, mantendo acesso total para `admin`.
- `RF-10` O sistema deve permitir cadastrar pessoas vinculadas a uma família, com nome, data de nascimento, marcação de `Richiedente` e observações.
- `RF-11` O sistema deve permitir editar e remover pessoas de uma família.
- `RF-12` Ao remover uma pessoa, o sistema deve remover também uniões e vínculos pai/mãe-filho associados a ela dentro da mesma família.
- `RF-13` O sistema deve permitir cadastrar uniões entre duas pessoas da mesma família.
...
- `RF-17` O sistema deve permitir cadastrar vínculos pai/mãe-filho entre pessoas da mesma família.
...
- `RF-21` O sistema deve gerar preview de layout automático da árvore para uma família com pessoas cadastradas.
...
- `RF-24` O sistema deve permitir exportar a árvore genealógica de uma família em PDF.
- `RF-25` A exportação deve registrar histórico com autor, formato, versão de template, arquivo gerado e data de criação.
- `RF-26` O sistema deve permitir listar exportações de uma família.
- `RF-27` O sistema deve permitir baixar um PDF exportado quando o usuário tiver acesso à família correspondente.
- `RF-28` O frontend em modo `live` deve permitir login, listagem de famílias, edição básica da família, preview e exportação.
...
- `RNF-10` O sistema deve preservar segregação de acesso por família entre usuários `admin` e `staff`.
