...
- As páginas principais em modo `live` são login, listagem de famílias e editor da família.
...
- Expõe rotas em `/api/v1` para autenticação, usuários, famílias, pessoas, preview e exportações.
- Usa dependências de autenticação para validar JWT e resolver o usuário atual.
- Aplica restrição de acesso por papel e por visibilidade da família.
...
- Permite criar famílias, pessoas, uniões, vínculos, visualizar preview e gerar exportações.
...
3. O usuário cria ou acessa uma família.
...
- `Family`: unidade principal de trabalho, com título, referência do cliente, status e autor.
- `Person`: pessoa vinculada a uma família, com nome, nascimento, marcação de `Richiedente` e observações.
- `Union`: relacionamento entre duas pessoas da mesma família.
...
- A visibilidade de famílias é central para o backend: `admin` acessa todas as famílias e `staff` apenas as próprias.
