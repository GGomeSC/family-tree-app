...
def create_parent_child_link(
    ...
    _assert_person_in_family(db, family_id, payload.parent_person_id)
    _assert_person_in_family(db, family_id, payload.child_person_id)

    existing_edges = [
        (l.parent_person_id, l.child_person_id)
        for l in db.query(ParentChildLink).filter(ParentChildLink.family_id == family_id).all()
    ]
    ...
    all_person_ids = {p.id for p in db.query(Person).filter(Person.family_id == family_id).all()}
    all_edges = [
        (l.parent_person_id, l.child_person_id)
        for l in db.query(ParentChildLink).filter(ParentChildLink.family_id == family_id).all()
    ]
